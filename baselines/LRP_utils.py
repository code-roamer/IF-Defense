...
def Add_Iden_to_conv(iden):
   B, C = iden.shape[0], iden.shape[1]
   newlayer = nn.Conv1d(C, C, 1)
   mat = numpy.zeros((9, 9)).astype(numpy.float32)
   mat[0, 0] = 1.0; mat[1, 1] = 1.0; mat[2, 2] = 1.0
   mat[3, 3] = 1.0; mat[4, 4] = 1.0; mat[5, 5] = 1.0
   mat[6, 6] = 1.0; mat[7, 7] = 1.0; mat[8, 8] = 1.0
   weight_mat = torch.from_numpy(mat).view(9, 9, 1).cuda()
   newlayer.weight = nn.Parameter(weight_mat)
   newlayer.bias = nn.Parameter(iden[0])

   return newlayer


def bmm_to_conv(mat):
   newlayers = [None]*mat.shape[0]
   m, n = mat.shape[1], mat.shape[2]
   mat = mat.transpose(2, 1)
   for i in range(len(newlayers)):
       newlayer = nn.Conv1d(m, n, 1)
       newlayer.weight = nn.Parameter(mat[i].reshape(n, m, 1))
       newlayer.bias = nn.Parameter(torch.zeros(n).cuda())
...
def toconv(layers):
    ...
    for i,layer in enumerate(layers):
        ...
        elif isinstance(layer, nn.Sequential) and isinstance(layer[0], nn.Linear):

            newlayer = None

            m,n = layer[0].weight.shape[1], layer[0].weight.shape[0]
            newlayer = nn.Conv1d(m, n, 1)
            newlayer.weight = nn.Parameter(layer[0].weight.reshape(n, m, 1))

            newlayer.bias = nn.Parameter(layer[0].bias)

            newlayers += [nn.Sequential(newlayer, layer[1])]
